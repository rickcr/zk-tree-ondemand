 ...
 */
public class RODTreeModel<T extends RODTreeNodeData> extends AbstractTreeModel<RODTreeNode<T>> implements TreeSelectableModel {
	...
	public int[] getPath (RODTreeNode<T> child) {
		System.out.println("getPath called for child "+child);
		...
		for (int i = 0; i < dPath.size(); i++) {
			path[i] = dPath.get(i);
			System.out.println(" .. set path["+i+"] to "+path[i]);
		}
